...
import { useChildMedicationDetailsForm } from "./useChildMedicationDetailsForm";
import { FRF1FormData } from ".";

const ChildMedicationDetailsForm = ({ action, id }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } =
  useChildMedicationDetailsForm();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme)=>style.headerTitle(theme)}>
          CHILD INCIDENTS FORM
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
...
export default ChildMedicationDetailsForm;
const style = {
