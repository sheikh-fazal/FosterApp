...
import { useCLADocumentationForm } from "./useCLADocumentationForm";
import { FRF1FormData } from ".";

const ClaDocumentationForm = ({ action, id }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } =
  useCLADocumentationForm();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme)=>style.headerTitle(theme)}>
          CLA DOCUMENTATION FORM
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
...
export default ClaDocumentationForm;
const style = {
