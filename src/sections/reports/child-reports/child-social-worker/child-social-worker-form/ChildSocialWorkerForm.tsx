...
import { useChildSocialWorkerForm } from "./useChildSocialWorkerForm";
import { FRF1FormData } from ".";

const ChildSocialWorkerForm = ({ action, id }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } =
  useChildSocialWorkerForm();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme)=>style.headerTitle(theme)}>
          CHILD SOCIAL WORKER FORM
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
...
export default ChildSocialWorkerForm;
const style = {
