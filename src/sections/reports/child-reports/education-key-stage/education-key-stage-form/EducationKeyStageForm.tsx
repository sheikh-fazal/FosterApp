...
import { useEducationKeyStageForm } from "./useEducationKeyStageForm";
import { FRF1FormData } from ".";

const EducationKeyStageForm = ({ action, id }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } =
  useEducationKeyStageForm();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme)=>style.headerTitle(theme)}>
          EDUCATION KEY STAGE FORM
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
...
export default EducationKeyStageForm;
const style = {
