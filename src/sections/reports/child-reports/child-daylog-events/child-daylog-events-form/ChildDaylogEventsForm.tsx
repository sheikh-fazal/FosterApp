...
import { useChildDaylogEventForm } from "./useChildDaylogEventsForm";
import { FRF1FormData } from ".";

const ChildDaylogEventForm = ({ action, id }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } =
  useChildDaylogEventForm();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme)=>style.headerTitle(theme)}>
          CHILD DAYLOG EVENTS FORM
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
...
export default ChildDaylogEventForm;
const style = {
