...
import { useChildMissingPlacementsForm } from "./useChildMissingPlacementsForm";
import { FRF1FormData } from ".";

const ChildMissingPlacementForm = ({ action, id }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } =
  useChildMissingPlacementsForm();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme)=>style.headerTitle(theme)}>
          CHILD MISSING PLACEMENT FORM
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
