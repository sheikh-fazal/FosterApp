              ...
              data.formFields.map((field: any) => (
                field.subFields.map((sub: any, i: number) => (
                  <Grid item md={6} xs={12} key={i}>
                    {field.title && <Typography sx={styles.title}>{field.title}</Typography>}
                    {sub.title && <Typography sx={styles.title}>{sub.title}</Typography>}
...
const styles = {
  title: {
    fontWeight: 500, fontSize: '16px', color: '#0E918C'
  }
}