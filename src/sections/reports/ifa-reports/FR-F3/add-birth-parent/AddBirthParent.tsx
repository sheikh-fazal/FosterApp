...
import { useAddBirthParent } from "./useAddBirthParent";
import { FRD1FormData } from ".";

const AddBirthParent = ({ action }: any) => {
  const disabled = action === "view" ? true : false;
  const { methods, onSubmit, handleSubmit, handleBack } = useAddBirthParent();
  return (
    <Card sx={{ p: 2 }}>
      <Box textAlign={"center"}>
      <Typography sx={(theme) => style.headerTitle(theme)}>
          FR-F3: BIRTH PARENT/BIRTH FAMILY MEMBER
        </Typography>
        <Typography
          sx={{
            fontWeight: 500,
