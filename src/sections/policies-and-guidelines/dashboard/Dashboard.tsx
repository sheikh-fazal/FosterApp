                ...
                {policyGuidelineCardsData.map((card, index) => (
                    <Grid item xl={3} lg={4} md={6} xs={12} key={index}>
                        <Link href={card.link} style={{ all: "unset" }}>
                            <Box sx={styles.policyCardsWrapper}>
                                <Box sx={{ flexBasis: "75%" }}>
...
const styles = {
    policyCardsWrapper: {
        background: "#0E918C", borderRadius: "10px", boxShadow: "0px 0px 7px 3px rgba(14, 145, 140, 0.2)", display: "flex", justifyContent: "space-between", padding: "15px", minHeight: "130px", height: "100%", cursor: "pointer"
    },
    cardsText: {
