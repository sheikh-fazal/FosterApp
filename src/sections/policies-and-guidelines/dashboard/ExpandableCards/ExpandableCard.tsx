    ...
    return (
        <Card sx={{ p: 1.5 }}>
            <Box display='flex' alignItems='center' gap='10px' mb='20px'>
                ...
                <Typography color={color} fontWeight="600" fontSize="18px">{title}</Typography>
            </Box>
                        ...
                        {card.list ? card.list.map((item: any) => (
                            <Box>
                                <Box display='flex' gap='15px' alignItems='center' mt="15px">
                                    ...
                                    <Typography color="7A8DA0" fontWeight="500" marginRight="20px">{item.time}</Typography>
                                    <Link href={item.href} style={{ all: "unset" }}>
                                        <Typography color={color} fontWeight="500" sx={{ cursor: "pointer" }}>{item.title}</Typography>
                                    </Link>
                                </Box>
                            </Box>
                        )) :
                            <Box display='flex' gap='15px' alignItems='center' mt="15px">
                                ...
                                {card.time && <Typography color="#7A8DA0" fontWeight="500">{card.time}</Typography>}
                                {card.title && <Link href={card.href} style={{ all: "unset" }}><Typography color={color} fontWeight="500" sx={{ cursor: "pointer" }}>{card.title}</Typography></Link>}
                            </Box>
                        }
                    </Box>
                ))}
...
const styles = {
    listBatch: ((color: string) => ({
        flexShrink: 0, backgroundColor: color, height: '15px', width: '15px', borderRadius: "3px"
    }))
}
