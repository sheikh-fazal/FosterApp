import { Avatar, Box, Button, Card, Grid, InputAdornment, LinearProgress, Rating, Select, TextField, Typography, useTheme } from '@mui/material'
              ...
              <Grid item lg={12} md={6} sm={6} xs={12} sx={{ mb: '20px' }}>
                {author.map(({ author, label }) => (
                  <Box display={'flex'} alignItems={'center'} key={label} sx={{ my: '14px' }}>
                    <Typography flexBasis={'20%'} sx={{ fontWeight: 600, fontSize: '16px', mr: '10px' }}>{label}</Typography>
    ...
    <Box sx={{ my: '20px' }}>
      <Box display={'flex'} alignItems={'center'} gap={'20px'} flexWrap={'wrap'}>
        ...
        <Typography>
          ...
          <Typography sx={{ ...styles.title, color: '#898989', display: 'flex', alignItems: 'center', gap: '15px', flexWrap: 'wrap' }}>
            <Rating value={rating} readOnly />
            <span>{dayPosted}</span>
          </Typography>
        </Typography>
...
const styles = {
  topSection: { display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 2, mb: 4, flexWrap: 'wrap' },
