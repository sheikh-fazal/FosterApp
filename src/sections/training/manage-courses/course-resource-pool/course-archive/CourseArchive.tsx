import { Avatar, Box, Button, Card, Grid, InputAdornment, LinearProgress, Rating, Select, TextField, Typography } from '@mui/material'
...
const CourseArchive = () => {
  ...
  const { archiveModal, DurationToHourMin, handleArchiveModal, isArchive } = useCourseArchive();
    ...
    return courses.map((course, i) => (
      {
        ...
        component: <Grid container spacing={3} sx={{ p: '10px 20px' }}>
          {course.component.map(({ title, type, duration, pages }: any, i) => (
            <Fragment key={i}>
              <Grid item md={9.8}>
                <Box display={'flex'} alignItems={'center'} gap={'15px'}>
                  <Image src={type === 'video' ? VideoPreviewIcon : PdfIcon} alt='icon' />
                  <Typography sx={{ ...styles.title, textTransform: 'capitalize' }}>{title}</Typography>
                </Box>
              </Grid>
              <Grid item md={2.2}>
                <Box display={'flex'} alignItems={'center'} gap={'15px'}>
                  <Typography sx={{ ...styles.title, fontWeight: 600, cursor: "pointer" }}>Preview</Typography>
                  <Typography sx={{ ...styles.title }}>{duration ? duration : `${pages} Pages`}</Typography>
                </Box>
              </Grid>
            </Fragment>
          ))}
        ...
        <Grid container>
          <Grid item md={4} xs={12}>
            ...
            {author.map(({ author, label }) => (
              <Box display={'flex'} alignItems={'center'} key={label} sx={{ my: '14px' }}>
                <Typography flexBasis={'20%'} sx={{ fontWeight: 600, fontSize: '16px' }}>{label}</Typography>
                <Typography sx={{ fontWeight: 600, fontSize: '14px', color: '#898989', textTransform: 'capitalize' }}>{author}</Typography>
              </Box>
            ))}
            <Box display={'flex'} alignItems={'center'} gap={'10px'}>
              <Image src={UpdateIcon} alt='icon' />
              <Typography sx={{ color: '#0E918C', fontWeight: 600, fontSize: '14px' }}>Last Updated 2/2023</Typography>
            </Box>
          </Grid>
          <Grid item lg={8} xs={12}>
            ...
            <Box sx={styles.topSection}>
              ...
              <Box>
                <Button variant='contained' onClick={handleArchiveModal}>Archive Course</Button>
              </Box>
            </Box>
            <Grid container spacing={2}>
              ...
              {requirements.map((data, i) => <Grid item key={i} xs={12}>
                <TitleWithBadge title={data.title} />
              </Grid>)}
              ...
              {description.map((data, i) => <Grid item key={i} xs={12}>
                <TitleWithBadge title={data.title} hideCircle />
              </Grid>)}

              <Grid item xs={12}>
                <Typography sx={{ ...styles.head, mb: '0px' }}>Course Content</Typography>
              </Grid>
              <Grid item xs={12}>
              ...
              <Grid item xs={12} display={'flex'} alignItems={'center'} gap={'20px'} flexWrap={'wrap'}>
              ...
              <Grid item xs={12} display={'flex'} gap={'20px'} flexWrap={'wrap'}>
                ...
                <Box>
                  <Typography sx={styles.userName}>de. neil thompson</Typography>
                  <TitleWithIcon title={'4.5 instructor rating'} icon={RatingStars} />
                ...
                <Typography sx={styles.title}>
                  Dr Neil Thompson is a highly experienced tutor and trainer. He is highly respected for his ability to explain complex ideas clearly and effectively. His gentle voice and unhurried approach are widely appreciated by online learners as will as attendees at his conferences and workshops. <br /><br />
              ...
              <Grid item xs={12}>
                <Grid container spacing={3} sx={{ my: '20px' }}>
                  <Grid item lg={4} md={6} xs={12}>
                    <Typography sx={{ ...styles.head, my: '0px' }}>Feedback & Rating</Typography>
                    <Typography sx={{ ...styles.rating }}>4.9</Typography>
                    <Rating value={4.9} readOnly />
                    <Typography sx={{ ...styles.courseRating, ...styles.title }}>Course Rating <span className='reviews'>(20 Reviews)</span></Typography>
                  </Grid>
              ...
              <Grid item xs={12}>
                <Grid container spacing={3}>
                  <Grid item lg={7} md={6} xs={12}>
                    <Typography sx={{ fontWeight: 600, fontSize: '16px' }}>Search Review</Typography>
                    <TextField
                      size='small'
                      fullWidth
                      InputProps={{
                  ...
                  <Grid item lg={5} md={6} xs={12}>
                    <Typography sx={{ fontWeight: 600, fontSize: '16px' }}>Label</Typography>
...
const TitleWithIcon = ({ title, icon }: any) => {
  return (
    <Typography sx={{ ...styles.title, display: 'flex', alignItems: 'center', gap: '5px', my: '3px' }}>
...
const Comments = ({ userProfile, userName, comment, rating, dayPosted }: any) => {
  comment = 'Excellent course material and materials are shared in depth for each topic. Highly recommended if you are a beginner!!!';
  ...
  return (
    <Box sx={{ my: '20px' }}>
      <Box display={'flex'} alignItems={'center'} gap={'20px'}>
        <Avatar src={userProfile} sx={{ height: '60px', width: "60px", background: "#1A202E", color: "white", }}>
          {firstName.charAt(0).toUpperCase()} {lastName.charAt(0).toUpperCase()}
        </Avatar>
        <Typography>
          <Typography sx={{ fontWeight: 700, fontSize: '16px', color: '#1A202E', textTransform: 'capitalize' }}>{userName}</Typography>
          <Typography sx={{ ...styles.title, color: '#898989', display: 'flex', alignItems: 'center', gap: '15px' }}>
            <Rating value={rating} readOnly />
            {dayPosted}
          </Typography>
        </Typography>
      </Box>
      <Typography sx={{ ...styles.title, color: '#343A40', mt: '24px', maxWidth: '500px' }}>{comment}</Typography>
    </Box>
  )
}
...
const ArchiveCourseMessage = ({ data }: any) => {
  return (
    <Box sx={styles.archiveCourseBox}>
      <Typography sx={{ mb: '5px' }}>This is an archived course and can not be edited.</Typography>
      <Grid container>
        {
          data.map(({ value, label }: any, i: number) => (
            ...
            >
              <Typography sx={{ color: '#81222C', fontWeight: 700, fontSize: '14px' }}>{label}</Typography>
              <Typography sx={{
                textTransform: label === 'User Name:' ? 'capitalize' : '',
                color: '#81222C', fontWeight: 400, fontSize: '14px'
              }}>
                {value}
              </Typography>
            </Grid>
...
const styles = {
  topSection: { display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 2, mb: 4 },
  head: { fontSize: "18px", fontWeight: 600, my: '20px' },
  title: { fontWeight: 500, fontSize: '14px' },
  circle: {
    height: 15, width: 15, borderRadius: 20, backgroundColor: 'red', mr: '20px', mt: 0.5,
    flexShrink: 0, background: 'linear-gradient(106.35deg, #0E918C 0%, #23E0D9 100%)'
  },
