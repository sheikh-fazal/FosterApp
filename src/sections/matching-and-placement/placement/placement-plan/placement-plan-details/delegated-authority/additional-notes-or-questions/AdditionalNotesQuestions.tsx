...
const AdditionalNotesQuestions = () => {
  ...
  const { noteValue, handleChange, handleBack } = useAdditionalNotesQuestions();

  return (
    <Card sx={{ p: 2 }}>
      <Typography sx={styles.title}>Additional notes or categories</Typography>
      <TextField
        label={'Please write here if you have any additional notes or questions'}
        fullWidth
        multiline
        minRows={20}
        value={noteValue}
      ...
      <Box display={'flex'} alignItems={'center'} gap={'15px'} marginTop={'20px'}>
        <LoadingButton type="submit" variant="contained" disabled={!noteValue}>
          Submit
        </LoadingButton>
