...
import { defaultValues, formSchema, formatters } from "./ChildMissingPlacementData";
...
export const useChildMissingPlacementForm = () => {
  const router = useRouter();
  const { action, id, fosterChildId } = router.query;
  const theme: any = useTheme();
  const [isLoading, setIsLoading] = useState(true);
  const [isFetching, setIsFetching] = useState(false);
  const [getChildMissingPlacementList] =
    useLazyGetChildChronologyOfEventsChildMissingPlacementByIdQuery();
  const [postChildMissingPlacementData] =
    usePostChildChronologyOfEventsChildMissingPlacementMutation({});
  const [editChildMissingPlacementList] =
    usePatchChildChronologyOfEventsChildMissingPlacementByIdMutation();

  const getDefaultValue = async () => {
    if (action === "view" || action === "edit") {
      const { data, isError } = await getChildMissingPlacementList({ id: id });
      setIsLoading(false);
      if (isError) {
  ...
  const onSubmit = async (data: any) => {
    if (action === "add") {
      setIsFetching(true);
      postChildMissingPlacementData({
        addChildMissingPlacementRequestDto: { ...data, fosterChildId, status: "Pending" },
      })
        .unwrap()
        .then((res: any) => {
          setIsFetching(false);
      ...
      const formData = {
        id,
        addChildMissingPlacementRequestDto: { ...data },
      };
      editChildMissingPlacementList(formData)
        .unwrap()
        .then((res: any) => {
