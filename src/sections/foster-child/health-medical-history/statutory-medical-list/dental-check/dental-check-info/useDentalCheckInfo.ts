...
import { defaultValueDentalCheckInfoForm, dentalCheckInfoFormDataFunction, dentalCheckInfoFormSchema } from ".";

export const useDentalCheckInfo = () => {
  const { query } = useRouter();
  const router = useRouter();
  
  const dentalCheckInfoFormData = dentalCheckInfoFormDataFunction(query?.action === "view");
...
const setGpDetailsInfoDefaultValue = async () => {
  ...
  if (isError) {
    return defaultValueDentalCheckInfoForm(data);
  }
  return defaultValueDentalCheckInfoForm(data?.data);
};

  const methods: any = useForm({
    resolver: yupResolver(dentalCheckInfoFormSchema) ,
