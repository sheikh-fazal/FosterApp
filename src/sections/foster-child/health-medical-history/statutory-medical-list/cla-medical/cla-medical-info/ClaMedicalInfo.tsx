...
import { useClaMedicalInfo } from "./useClaMedicalInfo";

const ClaMedicalInfo = () => {
  const { claMedicalInfoFormData, methods, handleSubmit, submitGpDetailsInfoForm, getSingleGpDetailsInfoDataStatus, query, router } =
    useClaMedicalInfo();
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(submitGpDetailsInfoForm)}>
      <Grid container spacing={5}>
        {claMedicalInfoFormData.map((form: any, index: number) => {
          return (
