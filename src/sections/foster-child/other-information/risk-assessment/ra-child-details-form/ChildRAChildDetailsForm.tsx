...
// import { RAChildDetailsFormFields } from "./RiskAssessmentData";
import FormProvider from "@root/components/hook-form/FormProvider";
import { RHFSelect, RHFTextField } from "@root/components/hook-form";
import { LoadingButton } from "@mui/lab";
// import { useRAChildDetailsForm } from "./useRAChildDetailsForm";
import SkeletonFormdata from "@root/components/skeleton/SkeletonFormdata";
import { useChildRAChildDetailsForm } from "./useChildRAChildDetailsForm";
import { RAChildDetailsFormFields } from ".";

// const backPath = "/foster-child/health-medical-history/child-medication-info";

const ChildRAChildDetailsForm = () => {
  const {
    router,
    methods,
    onSubmit,
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <Grid container sx={{ my: 2 }}>
          <Grid item xs={12} md={4}>
            Carer Name: <b>Not Placed</b>
          </Grid>
          <Grid item xs={12} md={4}>
            Placing Authority:<b> Redbridge</b>
          </Grid>
          <Grid item xs={12} md={4}>
            Ethnicity:<b> Pakistani</b>
          </Grid>
        </Grid>

        <Grid container sx={{ my: 2 }}>
          <Grid item xs={12} md={4}>
            Child’s LA Social Worker:<b> Nil</b>
          </Grid>
          <Grid item xs={12} md={4}>
            LA SW Email Address: <b>Nil</b>
          </Grid>
          <Grid item xs={12} md={4}>
            LA SW Contact Number:<b> Nil</b>
          </Grid>
        </Grid>
        <Grid container sx={{ my: 2 }}>
          <Grid item xs={12} md={4}>
            Disabilities:<b> Nil</b>
          </Grid>
          <Grid item xs={12} md={4}>
            Legal Status:<b> Nil</b>
          </Grid>
        </Grid>
        <Grid container rowSpacing={4} columnSpacing={5} alignItems="center">
          {RAChildDetailsFormFields.map((form: any) => {
            return (
...
export default ChildRAChildDetailsForm;
