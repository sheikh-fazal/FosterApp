...
import { useLazyGetInitialContactDataQuery, usePatchInitialContactDataMutation } from "@root/services/initial-stage/initial-contact-api/initialcontactApi";
...
function InitialContactForm() {
    ...
    const methods: any = useForm({
        ...
        defaultValues: async () => {
            const { data, isError }: any = await getInitialContactData(userID?.user?.userId, true);
            setIsLoading(false);
            if (isError) {
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Typography
                sx={{ color: theme.palette.grey[500], fontWeight: theme.typography.fontWeightMedium, mb: theme.spacing(2) }}
                variant="subtitle2">
                Initial Contact
            </Typography>
                ...
                >
                    <Box sx={{ display: "flex", alignItems: "center", marginBottom: 2 }}>
                        ...
                        <RHFRadioGroup
                            ...
                            label="Area Office"
                            size="small"
                            options={["WhatsApp", "Email", "Facebook", "Instagram", "Phone,", "Other"]}>
                        </RHFRadioGroup>
                    </Box>
