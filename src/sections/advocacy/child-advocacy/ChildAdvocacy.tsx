  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xl={5} xs={12}>
        <Card sx={{ p: 1.5 }}>
          <Typography color="#0E918C" fontWeight="600" fontSize="18px">
            Upcoming Meetings
          </Typography>
          <Box sx={{ maxHeight: expand ? "240px" : "250px", overflowY: "scroll" }}>
            {dataArray.map((card: any) => (
              <Box display="flex" gap="15px" alignItems="center" mt="22px">
                ...
                {
                  <Typography color={theme.palette.mode} fontWeight="500">
                    {card.title}
                  </Typography>
                }
              </Box>
          ...
          <Box display="flex" flexWrap="wrap" justifyContent="flex-end" marginTop="10px">
            {upcomingMeetingsData?.length > 4 && (
              <IconButton
                onClick={() => {
                  setExpand(!expand);
                }}
                sx={{ background: "rgba(19,15,38,0.5)", color: "white", padding: "0", "&:hover": { background: "rgba(19,15,38,0.5)" } }}
              >
          ...
          <Grid item xs={12}>
            <Card>
              ...
              <Box sx={styles.iconStyles}>
                <InfoIcon sx={{ width: "16px", height: "16px", color: "#0E918C" }} onClick={() => setShowChildStatistics(!showChildStatistics)} />
              </Box>
              {showChildStatistics && (
