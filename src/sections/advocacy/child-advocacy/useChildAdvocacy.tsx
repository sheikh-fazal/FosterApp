...
export const useChildAdvocacy = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (info: any) => (
        <Box>
          <Image width={40} height={40} style={{ objectFit: "cover" }} src={UserImg} alt="" />
        </Box>
      ),
    ...
    },
    {
      ...
      cell: (info: any) => (
        <Button
          type="button"
          variant="contained"
          onClick={() => router.push({ pathname: "/advocacy/child-advocacy/child-details", query: { id: info.row.original.id } })}
        >
          View Details
        </Button>
      ),
