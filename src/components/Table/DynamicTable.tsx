...
// ----------------------------------------------------------------------
// component
const DynamicTable = (props: TTable) => {
  const {
    columns,
    data,
    isFetching = false,
  ...
  let dynamiColumns = Object.keys(keys).map((data: any) => {
    return {
      accessorFn: (row: any) => {
        return row[data];
      },
  ...
  const colume = columns ?? dynamiColumns;
  ...
  return (
    <Grid container sx={{ position: "relative", ...rootSX }}>
      ...
      <Editrows columns={columns} setload={setload} />
    </Grid>
  );
};
...
const Editrows = (props: any) => {
  ...
  let defvalueArry: any = [];
  let defvalue: any = {};
  
  for (const obj of columns) {
    for (const id in obj) {
      if (id === "id") {
        defvalueArry.push(obj[id]);
        defvalue[obj[id]] = "";
      }
    }
  }
console.log(defvalue,"defvalue");
  ...
  return (
    <>
      <Button onClick={handleOpen}>Add rows</Button>
      <Modal
        open={open}
        onClose={handleClose}
        aria-labelledby="parent-modal-title"
      ...
      >
        <Box sx={styles.ModelRoot}>
          ...
          <FormProvider methods={methods} onSubmit={handleSubmit(SubmitData)}>
            <Grid container>
              {defvalueArry.map((data: any, index: number) => {
                console.log(data, "data");

                return (
                  <Grid key={index} xs={12} sx={{ my: 1 }} item>
                    <RHFTextField name={data} label={data} fullWidth={true} />
                  </Grid>
                );
              })}
            </Grid>
